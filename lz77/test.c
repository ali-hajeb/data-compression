...
int main() {
    ...
    if (run_command("gcc -c main.c -o main.o -Wall -g") != 0 ||
        run_command("gcc main.o -o main") != 0) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        // Run compression
        char cmd[MAX_PATH * 2];
        snprintf(cmd, sizeof(cmd), "./main -c %s", input_path);
        printf("TEST %d: Compressing %s\n", test_number, entry->d_name);
        ...
        snprintf(cmd, sizeof(cmd), "mv %s.lz7 %s", input_path, compressed_path);
        if (run_command(cmd) != 0) {
        ...
        // Run decompression
        snprintf(cmd, sizeof(cmd), "./main -d %s", compressed_path);
        printf("TEST %d: Decompressing %s\n", test_number, entry->d_name);
        if (run_command(cmd) != 0) {
        ...
        // Move decompressed file (assuming main -d creates <input>)
        char temp_decompressed[MAX_PATH];
        snprintf(temp_decompressed, MAX_PATH, "%s", compressed_path);
        snprintf(cmd, sizeof(cmd), "mv %s %s", temp_decompressed, decompressed_path);
        if (run_command(cmd) != 0) {
            fprintf(stderr, "Failed to move decompressed file for %s\n", entry->d_name);
            closedir(dir);
            return 1;
        }

        // Verify decompressed file matches original
        printf("TEST %d: Verifying %s\n", test_number, entry->d_name);
